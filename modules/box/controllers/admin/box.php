...
class box extends Admin_Controller
{
	...
	function index()
	{
		$data['box'] = $this->Box_model->get_boxes();
		$data['page_title']	= lang('boxes');
		
		
		$data['view_page']='box/admin/box/index';
	...
	function delete($id)
	{
		...
		$this->session->set_flashdata('message', lang('message_delete_boxes'));
		redirect(site_url('box/admin/box'));
	}
	
	/********************************************************************
	this function is called by an ajax script, it re-sorts the box
	********************************************************************/
	function organize()
	{
		$box = $this->input->post('box');
		$this->Box_model->organize($box);
	}
	...
	function form($id = false)
	{
		...
		if ($this->form_validation->run() == false)
		{
			$data['error'] = validation_errors();
				$data['view_page']='box/admin/box/form';
				$this->load->view($this->_container, $data);
		...
		{	
			...
			$this->session->set_flashdata('message', lang('message_boxes_saved'));
			
			redirect(site_url('box/admin/box/index'));
		}	
	}
