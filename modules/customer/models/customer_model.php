...
Class Customer_model extends MY_Model
{
	...
	function __construct()
	{
        ...
        $this->_TABLES=array('CUSTOMERS'=>$this->prefix.'customers',
            'CUSTOMERS_ADDRESS_BANK'=>$this->prefix.'customers_address_bank',
            'CUSTOMER_GROUPS'=>$this->prefix.'customer_groups'
        );
	...
	********************************************************************/
	...
	function get_customers($limit=0, $offset=0, $order_by='id', $direction='DESC')
	{
        ...
        $this->db->from($this->_TABLES['CUSTOMERS'].' customers');
	...
	{
		return $this->db->count_all_results($this->_TABLES['CUSTOMERS'].' customers');
	...
	function get_customer($id)
	{
		...
		$result	= $this->db->get_where($this->_TABLES['CUSTOMERS'].' customers', array('id'=>$id));
        return $result->row();
	}
	...
	function get_address_list($id)
	{
        ...
        $this->db->from($this->_TABLES['CUSTOMERS_ADDRESS_BANK'].' customers_address_bank');
	...
	function save_address($data)
	{
		...
		if(!empty($data['id']))
		{
			...
			$this->db->update($this->_TABLES['CUSTOMERS_ADDRESS_BANK'].' customers_address_bank', $data);
			return $data['id'];
		} else {
			$this->db->insert($this->_TABLES['CUSTOMERS_ADDRESS_BANK'], $data);
	...
	function delete_address($id, $customer_id)
	{
        ...
        $this->db->delete($this->_TABLES['CUSTOMERS_ADDRESS_BANK']);
		return $id;
	}
	...
	function save($customer)
	{
		if ($customer['id'])
		{
			...
			$this->db->update($this->_TABLES['CUSTOMERS'].' customers', $customer);
			return $customer['id'];
		}
		else
		{
			$this->db->insert($this->_TABLES['CUSTOMERS'], $customer);
			return $this->db->insert_id();
		}
	}
	...
	function delete($id)
	{
		...
		*/
		...
		$this->db->where('id', $id);
		$this->db->delete($this->_TABLES['CUSTOMERS']);
	...
	*/
	...
	{
		$result	= $this->db->get_where($this->_TABLES['CUSTOMERS'].' customer', array('email'=>$email));
	...
	function get_groups()
	{
        $this->db->from($this->_TABLES['CUSTOMER_GROUPS'].' customer_groups');
		$result=$this->db->get();
        return $result->result();
	}
	...
	{
		$this->db->where('id', $id);
		$this->db->delete($this->_TABLES['CUSTOMER_GROUPS']);
	}
	...
	function save_group($data)
	{
		...
		{
			$this->db->where('id', $data['id']);
            $this->db->update($this->_TABLES['CUSTOMER_GROUPS'].' customer_groups', $data);
			return $data['id'];
		} else {
