...
class Gift_card_model extends MY_Model 
{
	function __construct()
	{
		...
		$this->prefix='gc_';
        $this->_TABLES=array('GIFT_CARDS'=>$this->prefix.'gift_cards',
							'ORDERS'=>$this->prefix.'orders',
							'CANNED_MESSAGES'=>$this->prefix.'canned_messages'
	...
	function update_used_card_balances($gc_list)
	{
		foreach($gc_list as $code=>$card)
		{
			if(isset($card['amt_used'])) {
				...
				$this->db->update($this->_TABLES['GIFT_CARDS']);
			}
		}
	}
	...
	{
		$this->db->where('id', $id);
		$this->db->delete($this->_TABLES['GIFT_CARDS']);
	}
		
	function get_all_new() 
	{
		$this->db->select('gift_cards.*, orders.status', false);
		$this->db->from($this->_TABLES['GIFT_CARDS'].' gift_cards');
		$this->db->join($this->_TABLES['ORDERS'].' orders', 'gift_cards.order_number = orders.order_number', 'left');
	...
	{
		$this->db->insert($this->_TABLES['GIFT_CARDS'], $data);
	}
	...
	function send_notification($gc_data)
	{
		...
		$this->db->where('id', '1');
		$this->db->from($this->_TABLES['CANNED_MESSAGES'].' canned_messages');
