...
class Location extends Admin_Controller {	
	...
	function index()
	{
		...
		$data['locations']	= $this->Location_model->get_countries();
			...
			$data['view_page']='location/admin/location/index';
	...
	function country_form($id = false)
	{
		...
		{	
			...
			if (!$country)
			{
				...
				redirect(site_url('location/admin/location'));
			}
		...
		{
			$data['view_page']='location/admin/location/form';
			$this->load->view($this->_container, $data);
		}
		else
		{
	...
	function delete_zone($id = false)
	{
		...
		{	
			...
			else
			{
				...
				redirect(site_url('location/admin/location/zones/'.$location->country_id));
			}
		}
		else
		{
	...
	function zones($country_id)
	{
		...
		$data['view_page']='location/admin/location/country_zones';
		$this->load->view($this->_container,$data);
	}
	...
	function zone_form($id = false)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			$data['view_page']='location/admin/location/country_zone_form';
			$this->load->view($this->_container, $data);
		}
		else
		{
			...
			//go back to the product list
			redirect(site_url('location/admin/location/zones/'.$save['country_id']));
		}
	}
	...
	function zone_areas($id)
	{
		...
		$data['view_page']='location/admin/location/country_zone_areas';
		$this->load->view($this->_container, $data);
	...
	function delete_zone_area($id = false)
	{
		...
		{	
			...
			else
			{
				...
				redirect(site_url('location/admin/location/zone_areas/'.$location->zone_id));
			}
		}
		else
		{
	...
	function zone_area_form($zone_id, $area_id =false)
	{
		...
		{	
			...
			{
				$this->session->set_flashdata('error', lang('error_zone_area_not_found'));
				redirect(site_url('location/admin/location/zone_areas/'.$zone_id));
			}
		...
		if ($this->form_validation->run() == FALSE)
		{
			//$this->load->view('location/admin/country/country_zone_area_form';
			$data['view_page']='location/admin/location/country_zone_area_form';
		...
		else
		{
			...
			//go back to the product list
			redirect(site_url('location/admin/location/zone_areas/'.$save['zone_id']));
		}
	}
}