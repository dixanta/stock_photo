...
class Order extends Admin_Controller {	
	...
	function index($sort_by='order_number',$sort_order='desc', $code=0, $page=0, $rows=15)
	{
		...
		if($this->input->post('submit') == 'export')
		{
			...
			force_download_content('orders.xml', $this->load->view('order/admin/order/xml', $data, true));
		...
		$config['base_url']			= site_url('order/admin/order/index/'.$sort_by.'/'.$sort_order.'/'.$code.'/');
		...
		$data['view_page']='order/admin/order/orders';
	...
	function export()
	{
		...
		force_download_content('orders.xml', $this->load->view('order/admin/order/orders_xml', $data, true));
	}
		...
		foreach($data['order']->contents as $orderkey=>$product)
		{
			if(isset($product['is_gc']) && (bool)$product['is_gc'])
			{
				if($this->Gift_card_model->is_active($product['sku']))
				{
					...
					$data['order']->contents[$orderkey]['gc_status'] = ' [ <a href="'. base_url() . 'giftcard/admin/giftcard/activate/'. $product['code'].'">'.lang('activate').'</a> ]';
				}
			}
		}
		
		$data['view_page']='order/admin/order/index';
		$this->load->view($this->_container, $data);
	...
	function packing_slip($order_id)
	{
		...
		$data['view_page']='order/admin/order/packing_slip.php';
			$this->load->view($this->_container, $data);
	}
    ...
    function send_notification($order_id='')
    {
		...
		redirect(site_url('order/admin/order/views/'.$order_id));
	}
	...
	function bulk_delete()
    {
		...
		redirect(site_url('order/admin/order'));	
    }
}
