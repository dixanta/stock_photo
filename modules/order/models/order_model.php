...
Class order_model extends MY_Model
{
	function __construct()
	{
        ...
        $this->_TABLES=array('PAGES'=>$this->prefix.'pages',
							...
							'ORDER_ITEMS'=>$this->prefix.'order_items',
	...
	{	
		...
		$this->db->from($this->_TABLES['ORDERS'].' orders');
	...
	{			
		...
		return $this->db->count_all_results($this->_TABLES['ORDERS'].' orders');
	}
	...
	{	$this->db->from($this->_TABLES['ORDERS'].' orders');
		$this->db->join($this->_TABLES['ORDER_ITEMS'].' order_items', 'orders.id = order_items.id');
		$this->db->order_by('ordered_on', 'DESC');
		$this->db->where(array('customer_id'=>$id), 15, $offset);
		return $this->db->get()->result();
	}
	...
	function get_items($id)
	{
		...
		$this->db->from($this->_TABLES['ORDER_ITEMS'].' order_items');
		$result	= $this->db->get();
	...
	{
		$this->db->where('id', $id);
		$this->db->delete($this->_TABLES['ORDERS']);
	...
	function save_order($data, $contents = false)
	{
		if (isset($data['id']))
		{
			...
			$this->db->update($this->_TABLES['ORDERS'], $data);
			$id = $data['id'];
		...
		{
			$this->db->insert($this->_TABLES['ORDERS'], $data);
			...
			$this->db->where('id', $id);
			$this->db->update($this->_TABLES['ORDERS'], $order);
		...
		if($contents)
		{
			...
			$this->db->where('order_id', $id)->delete($this->_TABLES['ORDER_ITEMS']);
			// update order items
			foreach($contents as $item)
			{
				...
				$save['order_id']	= $id;
				$this->db->insert($this->_TABLES['ORDER_ITEMS'], $save);
			}
		}
	...
	function get_best_sellers($start, $end)
	{
		...
		foreach($orders as $order)
		{
			...
			$this->db->select('product_id, quantity')->where('order_id', $order->id);
			$this->db->from($this->_TABLES['ORDER_ITEMS'].' order_items');
