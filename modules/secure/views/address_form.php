...
<div class="modal" id="my-modal">
	<div class="modal-header">
		...
		<h3><?php echo lang('address_form');?>address_form</h3>
	</div>
	<div class="modal-body">
		...
		<div class="row-fluid">
			<div class="span12">
				<label><?php echo lang('address_company');?>address_company</label>
				<?php echo form_input($f_company);?>
			</div>
		</div>
		<div class="row-fluid">
			<div class="span6">
				<label><?php echo lang('address_firstname');?>address_firstname</label>
				<?php echo form_input($f_first);?>
			</div>
			<div class="span6">
				<label><?php echo lang('address_lastname');?>address_lastname</label>
				<?php echo form_input($f_last);?>
			</div>
		</div>
		<div class="row-fluid">
			<div class="span6">
				<label><?php echo lang('address_email');?>address_email</label>
				<?php echo form_input($f_email);?>
			</div>
			<div class="span6">
				<label><?php echo lang('address_phone');?>address_phone</label>
		...
		<div class="row-fluid">
			<div class="span12">
				<label><?php echo lang('address_country');?>address_country</label>
				<?php echo form_dropdown('country_id', $countries_menu, set_value('country_id', $country_id), 'id="f_country_id" class="span12"');?>
			</div>
		</div>
		<div class="row-fluid">
			<div class="span4">
				<label><?php echo lang('address_city');?>address_city</label>
				<?php echo form_input($f_city);?>
			</div>
			<div class="span6">
				<label><?php echo lang('address_state');?>address_state</label>
				<?php echo form_dropdown('zone_id', $zones_menu, set_value('zone_id', $zone_id), 'id="f_zone_id" class="span12"');?>
			</div>
			<div class="span2">
				<label><?php echo lang('address_postcode');?>address_postcode</label>
				<?php echo form_input($f_zip);?>
			</div>
		</div>
	</div>
	<div class="modal-footer">
		<a href="#" class="btn" data-dismiss="modal"><?php echo lang('close');?>CLOSE</a>
		<a href="#" class="btn btn-primary" type="button" onclick="save_address(); return false;"><?php echo lang('form_submit');?>Submit</a>
	</div>
</div>
...
$(function(){
	$('#f_country_id').change(function(){
			$.post('<?php echo site_url('location/admin/location/get_zone_menu');?>',{id:$('#f_country_id').val()}, function(data) {
			  $('#f_zone_id').html(data);
			});
		});
});
